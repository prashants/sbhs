...
if python_ver[0] == 2:
    print('This sbhsclient.py is specifically for Python version 3. Please use the sbhsclient.py for Python version 2')
    exit(1)
####if python_ver[1] < 6:
...
import urllib.request, urllib.error, urllib.parse, urllib.request, urllib.parse, urllib.error, http.cookiejar, socket
import hashlib
...
if user_use_proxy.lower() == 'yes':
    ...
    if user_proxy_username:
        ...
        proxy_support = urllib.request.ProxyHandler({"http" : "http://%(host)s:%(port)s" % proxy_info})

# cookie handling
cookie_support = http.cookiejar.CookieJar()

# enabling support for cookies and proxy
if user_use_proxy.lower() == 'yes':
    opener = urllib.request.build_opener(urllib.request.HTTPCookieProcessor(cookie_support), urllib.request.HTTPHandler, proxy_support)
else:
    opener = urllib.request.build_opener(urllib.request.HTTPCookieProcessor(cookie_support), urllib.request.HTTPHandler)
...
def authenticate():
    ...
    if not user_rollno:
        user_rollno = input('Username/Roll Number:')
    # get password
    ...
    try:
        ...
        postdata = urllib.parse.urlencode({'rollno' : user_rollno, 'password' : user_password})
        req = urllib.request.Request(url_auth)
        postdata = postdata.encode('utf-8')
        res = urllib.request.urlopen(req, postdata)
        content = res.read()
    except:
        print('Connection error ! Please check your internet connection.')
        return False
...
def initlogfiles():
    ...
    global cur_log_file
    try:
        open(scilabreadfname, 'w').close()
    except:
        print('Failed to create file: ' + scilabreadfname)
        return False
    try:
        open(scilabwritefname, 'w').close()
    except:
...
def startexperiment():
    ...
    # open the log files
    try:
        scilabreadf = open(scilabreadfname, 'w')
        scilabwritef = open(scilabwritefname, 'r')
        logf = open(cur_log_file, 'w')
    except:
        ...
        while True:
            ...
            while not srv_data:
                try:
                    ...
                    postdata = urllib.parse.urlencode({'iteration' : cur_iter, 'heat' : cur_heat, 'fan' : cur_fan, 'variables' : cur_variables, 'timestamp' : cur_time})
                    req = urllib.request.Request(url_com)
                    postdata = postdata.encode('utf-8')
                    res = urllib.request.urlopen(req, postdata)
                    content = res.read()
                    srv_data = True
