...
<li><a href="dirbrowser.php">Download Files</a></li>
...
mysql_select_db ("$db");
$stuff = mysql_query("SELECT * FROM `slot_booking` WHERE slot_date='".$date."' AND start_time='".$t1."' AND mid='".$mid."'") or die("MySQL Login Error: ".mysql_error()); 
...
mysql_select_db ("$db");
$stuff = mysql_query("SELECT * FROM `slot` WHERE start_time='".$t1."'") or die("MySQL Login Error: ".mysql_error()); 
...
$hash = md5($hash.$t1.$etime.$rollno);
echo "<center><h2><font color=green> Current slot is vacant </font><a href=current_booking.php?sh=$hash&st=$t1&et=$etime&rn=$rollno><img align=middle src=\"bn.gif\"></a></h2></center>";
...
<form class=\"safeSubmit\" 
method=\"post\" id=\"signup_form\" name=\"signup_form\" 
action=\"book_selecttime.php\">

<br><br>
	<p class=\"group\"><label for=\"rollno\">Select Date :</label><span 
class=\"help\">you will be allocated experiment slot on this date</span><span 
class=\"field\">
...
class=\"primary\">Select time slot >>></button></p>";
}
else
{
